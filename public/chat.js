...
import { state } from './main.js';
    ...
    export function applyTemplate(characterInfo) {
        // 메인 프롬프트 가져오기
        let finalPrompt = characterInfo.main_prompt_1 || "You are a helpful AI assistant.";
        
        // [대화프로필] - 프로필 이름 + 상세정보 결합
        const profileCombined = characterInfo.profile_name && characterInfo.profile_detail 
            ? `${characterInfo.profile_name} Information ${characterInfo.profile_detail}`
            : characterInfo.profile_name || characterInfo.profile_detail || '';
        
        // 플레이스홀더 치환
        finalPrompt = finalPrompt
            .replace(/\[캐챗제목\]/g, characterInfo.name || '')
            .replace(/\[대화프로필\]/g, profileCombined)
            .replace(/\[프로필이름\]/g, characterInfo.profile_name || '')
            .replace(/\[프로필정보\]/g, characterInfo.profile_detail || '')
            .replace(/\[상세프롬프트\]/g, characterInfo.prompt  || '')
            .replace(/\[유저노트\]/g, characterInfo.customSetting  || '');
        return finalPrompt;
    }
