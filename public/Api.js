...
function getThinkingConfigForModel(modelName) {
    if (!modelName) return null;
    if (modelName.startsWith('gemini-3')) return { thinkingLevel: "low" };
    if (modelName.startsWith('gemini-2.5')) return { thinkingBudget: 5000 };
    return null;
}
...
export async function sendToGemini(prompt, systemInstruction = "") {
    ...
    const thinkingConfig = getThinkingConfigForModel(modelName);
    console.log('[MainChat] generateContent request', { modelName, thinkingConfig, promptPreview: String(prompt).slice(0,120) });
    ...
    while (true) {
        ...
        try {
            const result = thinkingConfig
                ? await model.generateContent(prompt, { thinkingConfig })
...
export async function* sendToGeminiStream(prompt, history = [], systemInstruction = "") {
    ...
    while (true) {
        ...
        const modelName = getModelName();
        const thinkingConfig = getThinkingConfigForModel(modelName);
        console.log('[MainChat] streaming request', { modelName, thinkingConfig, historyCount: history.length, promptPreview: String(prompt).slice(0,120) });
        ...
        try {
            const streamOptions = {};
            if (thinkingConfig) streamOptions.thinkingConfig = thinkingConfig;

            const result = await chat.sendMessageStream(prompt, streamOptions);
