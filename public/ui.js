...
import { setModelName, getModelName } from './config.js';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (modelSelect) {
        ...
        modelSelect.addEventListener('change', function() {
            setModelName(this.value);
            showStatus(`모델이 ${this.options[this.selectedIndex].text}로 설정되었습니다.`, 'info');
        });
    }
});
...
window.saveApiKey = function() {
    ...
    if (!apiKey) {
        apiKeyStatus.textContent = 'API Key를 입력하세요';
